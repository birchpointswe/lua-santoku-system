...
static int tk_mutex_closure (lua_State *L)
{
  ...
  sem_wait(mutexp->sem);
  tk_lua_callmod(L, lua_gettop(L), LUA_MULTRET, "santoku.error", "pcall");
  sem_post(mutexp->sem);
  tk_lua_callmod(L, lua_gettop(L), LUA_MULTRET, "santoku.error", "checkok");
  return lua_gettop(L);
}
...
static int tk_mutex (lua_State *L)
{
  ...
  tk_mutex_t *mutexp = lua_newuserdata(L, sizeof(tk_mutex_t));
  mutexp->sem = sem;
  luaL_getmetatable(L, MT_MUTEX);
  lua_setmetatable(L, -2);
  lua_pushcclosure(L, tk_mutex_closure, 1);
  return 1;
}
